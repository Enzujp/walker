...
func newAPI() *walker.Router {
	...
	}, walker.Summary("Health"), walker.NoAuth())
	router.Route("/api", func(api *walker.Router) {
		api.Get("/users/{id}", bearer(func(w http.ResponseWriter, r *http.Request) {
			...
			write(w, 200, value)
		}), walker.Summary("Read user"), walker.Group("Users"), walker.PathValue("id", "42"), walker.Examples(
		...
		}), walker.Summary("Create user"), walker.Group("Users"), walker.HeaderValue("X-Trace", "{{trace}}"), walker.Examples(
		...
		}), walker.Summary("Search"), walker.HeaderValue("X-Trace", "{{trace}}"), walker.Query(
		...
		}, walker.Summary("Basic auth"), walker.Basic("{{username}}", "{{password}}"))
		...
		}, walker.Summary("API key header"), walker.APIKey("X-API-Key", "{{api_key}}", "header"))
		...
		}, walker.Summary("API key query"), walker.APIKey("api_key", "{{api_key}}", "query"))
		...
		}), walker.Summary("File"), walker.PathValue("name", "quarter report"), walker.PathValue("ext", "csv"))
		...
		}), walker.Summary("Encoded path"), walker.PathValue("value", "A B/ café"))
		...
		}), walker.Summary("Null payload"), walker.Body(map[string]string{"default": "value"}), walker.Examples(walker.RequestVariant{Name: "Inherit"}, walker.RequestVariant{Name: "Null", Body: json.RawMessage("null")}))
	})
	return router
}
